...
def _notify_abnormal_exit(bus: NotificationBus, sess: _TerminalSession, status: int | None) -> None:
    ...
    """
    shell = sess.shell or "shell"
    if status is None:
        title = "Terminal shell exited"
        body = f"The terminal process {shell} exited; its exit code is unknown."
    else:
        title = f"Terminal shell exited with code {status}"
        body = f"The terminal process {shell} exited with code {status}."
    try:
        bus.push(
            NotificationPayload(
                source="system",
...
async def api_terminal_ws(request: web.Request) -> web.WebSocketResponse | web.Response:
    ...
    if existing and not _sess_alive(existing):
        ...
        # owner socket). Answer it as an exit, never with a fresh shell: record
        # it and fall through to the exited-record answer below. The
        # reservation keeps concurrent opens at 409 across the awaits.
        ...
        try:
            ...
            raise
        _announce_exit(
            existing, status, bus=notification_bus, exited=exited_records, close=_close_on_exit()
        )
        registry.pop(session_id, None)
        existing = None
